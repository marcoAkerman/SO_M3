...
void rm(FILE *fp, char *filename, struct fat_bpb *bpb){
    ...
    printf("%d\n", wipe(fp, &file, bpb)); // Limpa os dados do arquivo

    file.name[0] = 0xE5; // Marca o primeiro byte do nome como excluído no diretório
...
void cp(FILE *fp, char *filename, struct fat_bpb *bpb){
    ...
    write_data(local_file, filename, dirs, bpb);
    //write_data(local_file, file.name, dirs, bpb);
